...
These values are designed to work with Home Assistant’s built-in charts and history graphs, but for a true 2D comparison chart you should use a custom time-series graph card that accepts numeric sensor values over time.
...
The built-in `history-graph` card is limited and does not render a true XY line chart for this use case. For a proper 2D chart, use a custom graph card that accepts multiple numeric series over time.

Example Lovelace YAML for a custom 2D chart card:
...
If your entity IDs differ slightly because of naming in your instance, adjust the entity names accordingly. This gives a true 2D chart for the three relevant values over time, so you can compare the total with the shared and non-shared components directly.
